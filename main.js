...
{
	if (region.U.H === region.Z.H)
	{
		return 'on line ' + region.U.H;
	}
	return 'on lines ' + region.U.H + ' through ' + region.Z.H;
}
...
var $author$project$Main$initialModel = {D: 0, E: '0', x: true, F: 32, G: '32', z: true};
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {aa: fragment, ac: host, ai: path, ak: port_, ap: protocol, aq: query};
	});
...
var $author$project$Main$cToF = function (c) {
	return (c * 1.8) + 32;
};
var $author$project$Main$fToC = function (f) {
	return (f - 32) / 1.8;
};
		...
		if (!msg.$) {
			...
			if (_v1.$ === 1) {
				...
				return _Utils_update(
					model,
					{
						D: $author$project$Main$fToC(number),
						x: true,
						F: number,
						G: userInput,
						z: true
			...
			if (_v2.$ === 1) {
				...
				return _Utils_update(
					model,
					{
						...
						E: userInput,
						x: true,
						F: $author$project$Main$cToF(number),
						z: true
					});
			}
		}
...
var $author$project$Main$FahrenheitToCelsius = function (a) {
	return {$: 0, a: a};
};
...
var $cuducos$elm_format_number$FormatNumber$Parser$FormattedNumber = F5(
	function (original, integers, decimals, prefix, suffix) {
		return {aH: decimals, ad: integers, ah: original, N: prefix, O: suffix};
	});
...
var $cuducos$elm_format_number$FormatNumber$Parser$classify = function (formatted) {
	...
	return onlyZeros ? 1 : ((formatted.ah < 0) ? 2 : 0);
};
	...
	function (locale, original) {
		...
		switch (_v0) {
			case 2:
				return _Utils_update(
					partial,
					{N: locale.M, O: locale.Q});
			case 0:
				return _Utils_update(
					partial,
					{N: locale.al, O: locale.am});
			default:
				return _Utils_update(
					partial,
					{N: locale.aB, O: locale.aC});
		}
	});
var $cuducos$elm_format_number$FormatNumber$Stringfy$formatDecimals = F2(
	function (locale, decimals) {
		return (decimals === '') ? '' : _Utils_ap(locale.r, decimals);
	});
var $cuducos$elm_format_number$FormatNumber$Stringfy$stringfy = F2(
	function (locale, formatted) {
		...
		var integers = A2($elm$core$String$join, locale.v, formatted.ad);
		var decimals = stringfyDecimals(formatted.aH);
		return $elm$core$String$concat(
			_List_fromArray(
				[formatted.N, integers, decimals, formatted.O]));
	});
...
var $author$project$Main$formatTemperature = function (_float) {
	return A2(
		$cuducos$elm_format_number$FormatNumber$format,
		_Utils_update(
			$cuducos$elm_format_number$FormatNumber$Locales$usLocale,
			{
				aH: $cuducos$elm_format_number$FormatNumber$Locales$Max(2)
			}),
		_float);
};
var $author$project$Main$viewCelsius = function (model) {
	return model.x ? $author$project$Main$formatTemperature(model.D) : model.E;
};
var $author$project$Main$viewFahrenheit = function (model) {
	return model.z ? $author$project$Main$formatTemperature(model.F) : model.G;
};
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Temperture Converter')
							])),
						A2(
						$elm$html$Html$h2,
						_List_fromArray(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text('Converts Fahrenheit to Celsius and Celsius to Fahrenheit. Type in numbers in either field and it\'ll convert the other one for you automatically.')
									])),
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$Attributes$href('https://github.com/JesterXL/temperature-converter-elm')
											]),
										_List_fromArray(
											[
												$elm$html$Html$text('https://github.com/JesterXL/temperature-converter-elm')
											]))
									]))
							])),
						A2(
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Events$onInput($author$project$Main$FahrenheitToCelsius)
							]),
						_List_Nil),
						A2(
						$elm$html$Html$label,
...
var $author$project$Main$main = $elm$browser$Browser$sandbox(
	{aO: $author$project$Main$initialModel, a_: $author$project$Main$update, a$: $author$project$Main$view});
